...
namespace {
SightRead::VocalTrack make_vocal_track(
...
BOOST_AUTO_TEST_CASE(perfect_vibrato_fills_pitched_tubes_faster)
{
    ...
    BOOST_CHECK_GT(phrase.pie.fill_between(phrase.start, first_half_end, true),
...
BOOST_AUTO_TEST_CASE(
    short_inter_phrase_gaps_do_not_create_activation_windows)
{
    const auto track = make_vocal_track(
        {{.position = SightRead::Tick {0},
          .length = SightRead::Tick {192},
...
BOOST_AUTO_TEST_CASE(fortnite_karaoke_full_esf_boosts_the_entire_phrase_under_od)
{
    ...
    BOOST_CHECK_CLOSE(path.activations.at(0).boosted_pie_fraction, 1.0,
                      0.0001);
    ...
    fortnite_karaoke_prefill_esf_reports_required_prefill_and_partial_boost)
{
    ...
    BOOST_CHECK_CLOSE(path.activations.at(0).boosted_pie_fraction, 0.4,
                      0.0001);
    ...
    fortnite_karaoke_activation_ending_mid_phrase_only_boosts_active_pie_fill)
{
    ...
    BOOST_CHECK_EQUAL(
        ...
        static_cast<int>(std::lround(song.phrases().at(1).base_score
                                     * path.activations.at(0)
...
BOOST_AUTO_TEST_CASE(rb_vocals_late_esf_can_full_boost_two_phrases)
{
    ...
    BOOST_CHECK_CLOSE(song.tempo_map()
                          .to_measures(late_activation.start)
                          .value(),
                      18.67, 0.01);
